...
def get_hashtag(GRANULARITY):
    ...
    if GRANULARITY == 'minutes':
         # timestamp - 1 minutes
        timestamp = datetime.timestamp(dateNow - timedelta(minutes= 2))
        # request each element with time > at timestamp
        for element in hashtag_collection.find( {"timestamp": {"$gt": timestamp}}):
        ...
        return jsonify({'result' : output})
         ...
         # timestamp - 1 hours
        timestamp = datetime.timestamp(dateNow - timedelta(hours= 1))
        # request each element with time > at timestamp
        for element in hashtag_collection.find( {"timestamp": {"$gt": timestamp}}):
            # add element (but object id must be serialized)
            output.append(json.dumps(element, cls=JSONEncoder))
        ...
        # timestamp - 1 month
        timestamp = datetime.timestamp(dateNow - timedelta(hours= 31*24*60))
        # request each element with time > at timestamp
        for element in hashtag_collection.find( {"timestamp": {"$gt": timestamp}}):
            # add element (but object id must be serialized)
            output.append(json.dumps(element, cls=JSONEncoder))
